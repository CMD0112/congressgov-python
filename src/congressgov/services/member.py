...
# NOTE: Expansion mappings for Member.expand() and extension methods
MEMBER_MAPPINGS = {
    "sponsoredLegislation": {member_sponsorship_list_sync: SponsoredLegislationModel},
    "cosponsoredLegislation": {member_cosponsorship_list_sync: CosponsoredLegislationModel},
}
...
__all__ = ['Member', 'MEMBER_MAPPINGS', 'MEMBER_PARAMETERS']


class Member(ApiService):
    """
    ...
    like expanding attributes and fetching sponsored/cosponsored legislation.
    ...
    def _fetch_members(
        ...
        *,
        ...
        if congress is not None and state is not None and district is not None:
            response = member_list_by_congress_state_district_sync(
            ...
            response = member_list_by_state_and_district_sync(
                client=resolved_client,
                state_code=state,
                district=district,
                format_=resolved_format,
            ...
            response = member_list_by_state_sync(
                client=resolved_client,
                state_code=state,
                format_=resolved_format,
                limit=limit,
    ...
    def _list_by_congress_page(
        ...
        *,
        ...
        response = member_congress_list_sync(
            client=client,
            congress=congress,
            format_=format_,
            offset=offset,
    ...
    # CUSTOM: current roster helper (not in generated service template)
