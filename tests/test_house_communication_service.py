"""HouseCommunication adoption: hand service."""
...
HAND = REPO_ROOT / "src" / "congressgov" / "services" / "house_communication.py"


def test_hand_house_communication_has_custom_hooks() -> None:
    src = HAND.read_text(encoding="utf-8")
    assert "class HouseCommunication" in src
    assert "# CUSTOM: validation" in src
    assert "# CUSTOM: universal search delegation" in src
    assert "validate_congress" in src
    assert "house_communication_detail_sync" in src



def test_house_communication_search_delegates_to_universal_search() -> None:
